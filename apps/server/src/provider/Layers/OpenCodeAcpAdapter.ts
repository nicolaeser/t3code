/**
 * OpenCode 2 sessions over ACP (`opencode acp`). OpenCode 1 keeps the HTTP SDK
 * adapter; this path is selected when the CLI reports 2.x.
 *
 * @module OpenCodeAcpAdapter
 */
...
const OPENCODE_ACP_RESUME_VERSION = 1 as const;

class OpenCodeAcpReplyTracker {
  push(_text: string) {}
...
export interface OpenCodeAcpAdapterLiveOptions {
   ...
   * Defaults to the built-in instance id (`opencode`).
   */
   ...
   * the OpenCode settings captured at construction.
   *
   ...
   */
  readonly resolveSettings?: Effect.Effect<OpenCodeSettings>;
...
interface OpenCodeAcpSessionContext {
  ...
  assistantReply: OpenCodeAcpReplyTracker;
  stopped: boolean;
}
...
export function makeOpenCodeAcpAdapter(
  ...
  return Effect.gen(function* () {
        ...
        Effect.mapError(
          (cause) =>
            new EffectAcpErrors.AcpTransportError({
              detail: "Failed to process OpenCode ACP event.",
              cause,
            }),
        ),
      );
    ...
    const stopSessionInternal = (ctx: OpenCodeAcpSessionContext) =>
      Effect.gen(function* () {
    ...
    const startSession: OpenCodeAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          let ctx!: OpenCodeAcpSessionContext;

          const resumeSessionId = parseOpenCodeAcpResume(input.resumeCursor)?.sessionId;
          ...
          // Resolve the OpenCodeSettings used to spawn the ACP child. Production
          ...
          // `updateSettings({ providers: { opencode: { binaryPath } } })` calls
          // actually take effect when the next session spawns.
          const effectiveOpenCodeSettings = options?.resolveSettings
            ? yield* options.resolveSettings
            : openCodeSettings;

          const mcpSession = McpProviderSession.readMcpProviderSession(input.threadId);
          const acp = yield* makeOpenCodeAcpRuntime({
            openCodeSettings: effectiveOpenCodeSettings,
          ...
          const session: ProviderSession = {
            ...
            threadId: input.threadId,
            resumeCursor: {
              schemaVersion: OPENCODE_ACP_RESUME_VERSION,
          ...
          ctx = {
            ...
            promptsInFlight: 0,
            assistantReply: new OpenCodeAcpReplyTracker(),
            stopped: false,
          };
          ...
          const nf = yield* Stream.runDrain(
            ...
            Effect.catch((cause) =>
              Effect.logError("Failed to process OpenCode runtime notification.", { cause }),
            ),
        ...
        return yield* Effect.gen(function* () {
          ...
          if (input.attachments && input.attachments.length > 0) {
            for (const attachment of input.attachments) {
              // OpenCode ACP image attachments only. Generic files reach the
            ...
            .prompt({
              prompt: /^\/[^\s/]+(?:\s|$)/.test(rawPrompt)
                ...
                : [
                    ...
                    {
                      type: "text",
                      text: buildRuntimeInstructions({ harness: "OpenCode", model: resolvedModel }),
                    },
                  ],
            })
            .pipe(
          ...
          if (ctx.promptsInFlight === 1 && result.stopReason !== "cancelled" && failure) {
            return yield* new ProviderAdapterRequestError({
              provider: PROVIDER,
              method: "session/prompt",
              detail: "OpenCode reported a transport failure.",
              cause: failure,
            });
          }
    ...
    const rollbackThread: OpenCodeAdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        ...
        return yield* new ProviderAdapterRequestError({
          ...
          detail: "OpenCode ACP sessions do not support provider-side rollback.",
        });
      });
    ...
    yield* Effect.addFinalizer(() =>
      Effect.forEach(sessions.values(), stopSessionInternal, { discard: true }).pipe(
        Effect.catch((cause) =>
          Effect.logError("Failed to emit OpenCode session shutdown event.", { cause }),
        ),
